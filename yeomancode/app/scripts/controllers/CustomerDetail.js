 ...
 angular.module('redPandaApp').controller('customerDetailController', ['$scope','$rootScope','$modal','$http','$location','$cookieStore','$filter','$anchorScroll', 'CurrentTimeStamp','$timeout','FilterDeleted','USDateFormat', 
    ...
    if ($rootScope.localCache.countries == null) {
        $http.get('/api/listData/countries').success(function(data) {
            ...
            console.log("No data found for countries");
    ...
    if ($rootScope.localCache.states == null) {
        $http.get('/api/listData/states').success(function(data) {
            ...
            console.log("No data found for states");
            //Code used for local testing and it should be removed finally.
    ...
    $scope.deleteContractList = function(obj, index) {
        ...
        $scope.closeAlert();
        $rootScope.showModal('/api/delete/contract/' + deleteObj.id + '?timestamp=' + CurrentTimeStamp.postTimeStamp(), 'Confirm Delete', 'Are you sure you would like to delete ' + deleteObj.title + '? This action can not be undone.', 'Cancel', 'Confirm');
    ...
    $scope.deleteContactList = function(obj, index) {
        var deleteObj = obj.entity;
        $rootScope.indexValue = index;
        $rootScope.showModal('/api/delete/contact/' + deleteObj.id + '?timestamp=' + CurrentTimeStamp.postTimeStamp(), 'Confirm Delete', 'Are you sure you would like to delete ' + deleteObj.name + ' ? This action can not be undone.', 'Cancel', 'Confirm');
        $scope.$watch('isPostSuccess', function(nValue, oValue) {
            if (nValue == null || (nValue == oValue))
                return;
            if ($rootScope.isPostSuccess) {
     ...
     */
     if ($rootScope.localCache.currencies == null) {
        $http.get('/api/listData/currencies').success(function(data) {
            ...
            console.log("No data found for currencies");
            //Code used for local testing and it should be removed finally.
     ...
     * contact pop up from customer screen
     * ======================================================================================================
     */
    ...
    $scope.openContactInfo = function(obj, index) {
       ...
       $rootScope.modalInstance = $modal.open({
            //templateUrl: '/dist/views/ContactPopUp.html'
